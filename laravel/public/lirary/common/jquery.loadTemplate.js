(function ($) {
    ...
    function bindData(template, data, settings) {
        ...
        // 自定义
        // processElements2(template, data, settings, function ($elem, value, attr) {
        //
        //     $elem.attr( attr, applyFormatters($elem, value, attr, settings));
        // });
    ...
    function processElements2( template, data, settings, dataBindFunction, noDataFunction) {
        ...
        $( template).each(function () {
            ...
            // var attribute = $this.data()

            $.each(nodes($this), function (i, v) {
                console.log(v)
                var param = $(v).data(),
                    value = getValue(data, param);
                ...
                if (typeof value !== 'undefined' && dataBindFunction) {
                    dataBindFunction($this, value, attribute);
                } else if (noDataFunction) {
                    noDataFunction($this);
                }
            })
